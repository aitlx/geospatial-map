...
import { ClipboardCheck, Eye, EyeOff, Lock, LogIn, Mail, ShieldCheck } from "lucide-react";
	...
	useEffect(() => {
		...
		const checkLocal = () => {
			...
			try {
				...
				console.debug("Admin login local session parse failed", error);
				return null;
			}
		};
		...
		const verifySession = async () => {
			try {
				...
				console.debug("Admin login session verification failed", error);
			} finally {
				if (isMounted && !redirected) {
					setCheckingSession(false);
				}
				...
				<section className="relative flex flex-col gap-4 rounded-3xl border border-emerald-100 bg-white/95 px-5 py-6 text-slate-900 shadow-xl backdrop-blur">
					<div className="relative z-10 flex flex-col gap-4">
						<div className="space-y-2">
							...
							<div>
								<h2 className="text-lg font-semibold text-slate-900">Manage municipal oversight</h2>
						...
						<div className="flex items-start gap-2.5 rounded-xl border border-emerald-100 bg-emerald-50/70 px-3 py-2.5 text-xs text-emerald-700">
							...
							<p className="leading-snug">Only elevated accounts can access municipal controls. Activity is logged.</p>
						</div>
						...
						<form onSubmit={handleSubmit} className="flex flex-col gap-4">
							<div className="space-y-1.5">
								...
								<div className={`flex items-center gap-2.5 rounded-xl border border-emerald-100 bg-emerald-50/70 px-3 py-2 shadow-sm transition ${
									submitting ? "opacity-80" : "focus-within:border-emerald-400 focus-within:bg-white focus-within:shadow-md"
									}`}>
							...
							<div className="space-y-1.5">
								...
								<div className={`flex items-center gap-2.5 rounded-xl border border-emerald-100 bg-emerald-50/70 px-3 py-2 shadow-sm transition ${
									...
									<button
										...
										className="flex h-6 w-6 items-center justify-center rounded-full text-emerald-500 transition hover:bg-emerald-50 disabled:opacity-60"
										disabled={submitting}
						...
						<div className="space-y-1.5 text-[11px] text-slate-500">
							<p>Need elevated access? Coordinate with a superadmin.</p>
							<div className="flex items-center gap-3">
								<Link to="/forgot-password" state={{ portal: 'admin' }} className="font-semibold text-emerald-600 transition hover:text-emerald-700">Forgot password?</Link>
								<Link to="/login" className="font-semibold text-emerald-600 transition hover:text-emerald-700">Switch to technician sign-in</Link>
							</div>
						</div>
					</div>
				</section>
